...
export default {
    ...
    methods: {
        ...
        GetInfo(pageNo) {
            ...
            this.$blockUI.Start();
            this.$http
                .GetBankBranches(this.pageNo, this.pageSize, this.BankId)
                .then((response) => {
                    this.$blockUI.Stop();
        ...
        submitForm(formName) {
            this.$refs[formName].validate((valid) => {
                if (valid) {
                    ...
                    this.$blockUI.Start();
                    this.$http.AddBankBranches(this.ruleForm)
                        .then(response => {
                            this.$blockUI.Stop();
        ...
        OpentEditDialog(item) {
            ...
            this.ruleForm.BankId = item.cityId;
        ...
        submitEditForm(formName) {
            this.$refs[formName].validate((valid) => {
                if (valid) {
                    ...
                    this.$blockUI.Start();
                    this.$http.EditBankBranches(this.ruleForm)
                        .then(response => {
                            this.$blockUI.Stop();
                            this.resetForm(formName);
                            this.GetInfo();
        ...
        Delete(Id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    this.$blockUI.Start();
                    this.$http.DeleteBankBranches(Id)
                        .then((response) => {
                            this.$blockUI.Stop();
                            this.$helper.ShowMessage('success', 'عملية ناجحة', response.data);
                            this.GetInfo();
