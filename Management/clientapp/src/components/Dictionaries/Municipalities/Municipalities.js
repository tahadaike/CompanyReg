...
export default {
    ...
    methods: {
        ...
        GetInfo(pageNo) {
            ...
            this.$blockUI.Start();
            this.$http
                .GetMunicipalities(this.pageNo, this.pageSize, this.CityId)
                .then((response) => {
                    this.$blockUI.Stop();
        ...
        submitForm(formName) {
            this.$refs[formName].validate((valid) => {
                if (valid) {
                    ...
                    this.$blockUI.Start();
                    this.$http.AddMunicipalities(this.ruleForm)
                        .then(response => {
                            this.$blockUI.Stop();
        ...
        OpentEditDialog(item) {
            ...
            this.ruleForm.CityId = item.citiesId;
        ...
        submitEditForm(formName) {
            this.$refs[formName].validate((valid) => {
                if (valid) {
                    ...
                    this.$blockUI.Start();
                    this.$http.EditMunicipalities(this.ruleForm)
                        .then(response => {
                            this.$blockUI.Stop();
                            this.resetForm(formName);
                            this.GetInfo();
        ...
        Delete(Id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    this.$blockUI.Start();
                    this.$http.DeleteMunicipalities(Id)
                        .then((response) => {
                            this.$blockUI.Stop();
                            this.$helper.ShowMessage('success', 'عملية ناجحة', response.data);
                            this.GetInfo();
